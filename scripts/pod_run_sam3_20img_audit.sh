...
MANIFEST="saved_activations/sam3_concept_masks/cub_sam3_concept_masks_base_20img_audit_v1/train/manifest.json"
...
python scripts/generate_sam3_concept_masks.py \
  --config configs/sam3/cub_concept_masks_base_20img_audit_pod.json \
  --split train \
  --run \
  --max_images 20 \
  --max_concepts 3 \
...
p = Path("saved_activations/sam3_concept_masks/cub_sam3_concept_masks_base_20img_audit_v1/train/manifest.json")
