...
Model: {model_name}
Checkpoint class source: {class_source}
Reference class for contribution: {reference_class}
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Export a judge-ready subset of concept-image pairs with top positively "
            "contributing concepts and native-map overlays for SALF/SAVLG checkpoints."
        )
    )
    ...
    parser.add_argument(
        ...
        help="How to pick the image subset from the evaluation split.",
    )
    parser.add_argument(
        "--class_source",
        type=str,
        default="pred",
        choices=["pred", "gt"],
        help="Class whose concept contributions are used for ranking.",
    )
    parser.add_argument(
        "--savlg_score_source",
        type=str,
        ...
        help="Raw SAVLG activation source used for the positive-activation filter.",
    )
    parser.add_argument(
        "--map_normalization",
        type=str,
        ...
        help="Normalization used only for saved judge overlays.",
    )
    parser.add_argument(
        "--min_positive_concepts",
        type=int,
        default=1,
        help="Skip images with fewer than this many positive concepts.",
    )
...
def _ensure_spatial_model(model_name: str) -> None:
    if model_name not in {"salf_cbm", "savlg_cbm"}:
        raise NotImplementedError(
            f"export_concept_judge_subset currently supports only salf_cbm/savlg_cbm, got {model_name}."
        )
...
def main() -> None:
    ...
    logger.info(
        "Exporting concept judge subset: model={} split=test requested_images={} topk_concepts={} class_source={} score_source={}",
        model_name,
    ...
    for images, concept_one_hot, targets in tqdm(loader, desc="Export concept judge subset"):
                ...
                judge_tasks.append(
                    {
                        ...
                        "checkpoint_path": cli_args.load_path,
                        "class_source": cli_args.class_source,
                        "reference_class": class_names[reference_class_idx],
                        "image_file": str(image_rel),
                        "overlay_file": str(overlay_rel),
                        "concept_name": concept_name,
                        "metadata": {
    ...
    logger.info(
        "Saved judge subset to {} with {} images and {} concept tasks",
        output_dir,
        selected_count,
        len(judge_tasks),
    )
